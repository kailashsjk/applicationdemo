microservice.service("shippingService", function($http, $location,$cookieStore) {
	...
	this.getShippingStatus=function(jwttoken,orderId,callback){
		var responsePromise = $http({	url:"http://localhost:9090/shipping-service/shipping/getShippingStatus?orderId="+orderId,
			method: "GET",
	...
	this.saveShipping = function(orderId, customerid, customerName, email,
		...
		var responsePromise = 	 $http({	url: "http://localhost:9090/shipping-service/shipping/create", 
			method: "POST", 
			data: shipping,
			headers: { 	'Content-Type': 'application/json',
				/*'Access-Control-Allow-Origin': 'http://localhost:9090',
