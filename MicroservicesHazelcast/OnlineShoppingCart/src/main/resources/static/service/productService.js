microservice.service("productService", function($http, $location,$cookieStore) {
	...
	this.getdata = function(jwttoken,callbackData) {
		...
		var responsePromise = $http({	url:"http://localhost:9090/catalogue-service/catalogueService/read-all",
			method: "GET",
	...
	this.updateProduct=function(id,availableItems,jwttoken,callback){
		...
		var productUpdate = {
			...
			var responsePromise = 	 $http({	url: "http://localhost:9090/catalogue-service/catalogueService/update", 
				method: "POST", 
				data: productUpdate,
				headers: { 	'Content-Type': 'application/json',
							'Access-Control-Allow-Origin': 'http://localhost:9090',
